    ...
    <div className="h-full flex flex-col overflow-hidden bg-black text-[#f0f0f0]">
      ...
      >
        <div className="h-full max-w-[600px] mx-auto px-3 flex items-center gap-4">
          ...
          <span className="text-[14px] font-black uppercase tracking-widest text-white">Chisme</span>
        </div>
      </motion.header>
        ...
        {loading ? (
          <div className="px-4 pt-5 pb-4 border-b border-[#181818]">
            <div className="flex gap-3 mb-4">
              <motion.div animate={{ opacity: [0.04, 0.1, 0.04] }} transition={{ repeat: Infinity, duration: 1.8 }} className="w-10 h-10 bg-white shrink-0" style={{ borderRadius: 0 }} />
              <div className="flex-1 flex flex-col gap-2">
                <motion.div animate={{ opacity: [0.04, 0.1, 0.04] }} transition={{ repeat: Infinity, duration: 1.8, delay: 0.1 }} className="h-2.5 bg-white w-24" />
              </div>
            </div>
            <motion.div animate={{ opacity: [0.04, 0.1, 0.04] }} transition={{ repeat: Infinity, duration: 1.8, delay: 0.15 }} className="h-3 bg-white w-full mb-2" />
            <motion.div animate={{ opacity: [0.04, 0.1, 0.04] }} transition={{ repeat: Infinity, duration: 1.8, delay: 0.2 }} className="h-3 bg-white w-4/5" />
          </div>
        ) : chisme ? (
          <>
            {/* ── Post original ── */}
              ...
              <article className="px-4 pt-5 pb-4 border-b border-[#181818]">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <ViewTransition name={`avatar-${chisme.id}`}>
                    <Avatar seed={chisme.avatar_seed} size={44} className="overflow-hidden shrink-0" style={{ borderRadius: 0 }} />
                  </ViewTransition>
                  <div>
                    <p className="text-[12px] font-black uppercase tracking-widest text-white">{chisme.username}</p>
                    <p className="text-[11px] text-[#383838] mt-0.5">{timeAgo(chisme.created_at)}</p>
                  </div>
                </div>

                {/* Texto grande */}
                <p className="text-[20px] text-[#ececec] leading-[1.65] whitespace-pre-wrap break-words mb-4">
                  {chisme.texto}
                </p>
                ...
                <AnimatePresence>
                  {hasStats && (
                    <motion.div
                      ...
                      className="flex gap-5 pb-3 border-b border-[#181818] mb-1"
                    >
                      {chisme.comment_count > 0 && (
                        <span className="text-[13px] text-[#383838]">
                          <span className="text-white font-black">{chisme.comment_count}</span> Comentarios
                        </span>
                      )}
                      {chisme.like_count > 0 && (
                        <span className="text-[13px] text-[#383838]">
                          <span className="text-white font-black">{chisme.like_count}</span> Likes
                        </span>
                      )}
                      {chisme.repost_count > 0 && (
                        <span className="text-[13px] text-[#383838]">
                          <span className="text-white font-black">{chisme.repost_count}</span> Reposts
                        </span>
                      )}
                    </motion.div>
                  )}
                ...
                <div className="flex items-center justify-around pt-1">
                  ...
                  >
                    <motion.div animate={isLiked ? { scale: [1, 1.35, 1] } : { scale: 1 }} transition={{ duration: 0.22 }}>
                      {isLiked
                        ? <HeartSolid width={18} height={18} style={{ color: BRAND }} />
                        : <Heart width={18} height={18} color="#404040" />
                      }
                    </motion.div>
                  ...
                  <div className="flex items-center gap-2 py-2 px-3">
                    <MessageText width={18} height={18} style={{ color: BRAND }} />
                    <CounterFlip count={chisme.comment_count} active={true} large />
                  </div>
                  ...
                  >
                    <motion.div animate={{ rotate: isReposted ? 360 : 0 }} transition={{ duration: 0.4 }}>
                      <Refresh width={18} height={18} color={isReposted ? BRAND : '#404040'} />
                    </motion.div>
            ...
            <motion.div variants={staggerContainer} initial="hidden" animate="show">
              <AnimatePresence initial={false}>
                {comentarios.map((c, idx) => (
                  <motion.div
                    ...
                    initial="hidden"
                    animate="show"
                    className="relative px-4 py-4 flex gap-3 border-b border-[#181818] last:border-0"
                  >
                    {/* Thread line al primer comentario */}
                    {idx === 0 && (
                      <motion.div
                        initial={{ scaleY: 0 }}
                        animate={{ scaleY: 1 }}
                        ...
                        className="absolute left-[34px] top-0 w-px h-4 bg-[#1c1c1c]"
                      />
                    )}

                    <div className="shrink-0 mt-0.5">
                      <Avatar seed={c.avatar_seed} size={32} className="overflow-hidden" style={{ borderRadius: 0 }} />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[14px] text-[#b0b0b0] leading-[1.6] whitespace-pre-wrap break-words">
                        {c.texto}
                      </p>
                    </div>
                  </motion.div>
              ...
              {comentarios.length === 0 && (
                <motion.p
                  ...
                  className="py-10 text-center text-[11px] font-bold uppercase tracking-widest text-[#1c1c1c]"
                >
                  Sin comentarios aún.
                </motion.p>
              )}
          ...
          <p className="py-10 text-center text-[11px] font-bold uppercase tracking-widest text-[#1c1c1c]">
            Chisme no encontrado.
          </p>
        )}
      </div>
      </div>
      ...
      >
        <div className="max-w-[600px] mx-auto h-full px-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            {chisme && !texto && (
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#252525] mb-0.5">
                → {chisme.username}
              </p>
            )}
            <input
              ...
              maxLength={300}
              className="w-full bg-transparent text-[14px] text-[#f0f0f0] placeholder-[#282828] outline-none"
            />
          </div>
          <AnimatePresence>
            {texto.trim() && (
