...
import { getChismesByUser, darLike, darRepost, type Chisme } from '@/lib/api'
...
export default function UserProfilePage() {
  ...
  const avatarSeed = chismes[0]?.avatar_seed ?? username
    ...
    <div className="h-full flex flex-col overflow-hidden bg-paper text-ink">
      ...
      >
        <div className="h-full max-w-[600px] mx-auto px-3 flex items-center gap-4">
          ...
          <span className="text-[13px] font-black uppercase tracking-widest text-ink">perfil</span>
        </div>
      </motion.header>
      ...
      <div className="flex-1 overflow-y-auto ruled">
        <div className="max-w-[600px] mx-auto">
          ...
          <motion.div
            ...
            className="px-5 py-6 border-b border-line flex items-center gap-4"
          >
            <Avatar seed={avatarSeed} size={52} frame="polaroid" className="shrink-0" />
            <div>
              <p className="font-hand-title text-[28px] leading-tight text-ink">{username}</p>
              {!loading && (
                <p className="text-[11px] font-mono text-ink-faint mt-0.5">
                  {chismes.length} {chismes.length === 1 ? 'chisme' : 'chismes'} en el cuaderno
                </p>
              )}
            </div>
          </motion.div>
          ...
          {loading ? (
            ...
            >
              <p className="font-hand text-[20px] text-ink-faint">
                {username} no ha chismeado nada todavía.
              </p>
            </motion.div>
          ) : (
